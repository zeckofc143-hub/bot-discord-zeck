...
const fmtAge=n=>{n=Math.round(n);if(n===0)return'Acabou de nascer';if(n===1)return'1 ano';return`${n.toLocaleString('pt-BR')} anos`};
const withUnknown=a=>[...a.map(fmtAge),'Desconhecida'];
...
function agesFor(r){
 // Regra da roleta: Humano tem cada idade individual de recém-nascido até 1.000, depois Desconhecida.
 if(r==='Humano')return withUnknown(seq(0,1000,1));
 if(cosmic.has(r))return withUnknown(uniq([...seq(0,1000,1),...seq(1010,10000,10),...seq(10100,100000,100),...seq(101000,1000000,1000),...seq(1010000,10000000,10000)]));
 if(ancient.has(r))return withUnknown(uniq([...seq(0,1000,1),...seq(1010,5000,10),...seq(5050,50000,50)]));
 if(longLived.has(r))return withUnknown(uniq([...seq(0,1000,1),...seq(1010,5000,10),...seq(5050,10000,50)]));
 if(shortLived.has(r))return withUnknown(seq(0,1000,1));
 if(mortal.has(r))return withUnknown(seq(0,1000,1));
 return withUnknown(seq(0,1000,1));
}
